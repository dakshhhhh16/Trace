...
// GetManifest generates and returns just the SBOM object (Manifest)
func GetManifest(src source.Source, defaultTags ...string) *sbom.SBOM {
	cfg := syft.DefaultCreateSBOMConfig()

	manifest, err := syft.CreateSBOM(context.Background(), src, cfg)
	if err != nil {
		panic(err)
	}
...
// SaveManifestToFile converts Manifest to CycloneDX format and saves to file
func SaveManifestToFile(manifest *sbom.SBOM, filePath string) error {
	cyclonedxFormatSbom, err := ToCycloneDxSchema(manifest)
	if err != nil {
		return fmt.Errorf("failed to convert Manifest to CycloneDX format: %w", err)
	}
...
func GetAllVulnAndUpload(s *sbom.SBOM) (string, error) {
	...
	log.Printf("[INFO] Database built at: %v", scanner.dbStatus.Built)
	log.Printf("[INFO] Database location: %v", scanner.dbStatus.Path)
	...
	log.Printf("[INFO] Converted %d packages for vulnerability scanning", len(grypePkgs))
	...
	log.Printf("[INFO] Found %d vulnerabilities (%d ignored)", matchesCollection.Count(), len(ignoredMatches))
	...
	log.Printf("[INFO] Vulnerabilities saved to: %s", vulnFilePath)
	return vulnFilePath, nil
}

func GetManifestData(src source.Source, defaultTags ...string) sbom.SBOM {
	// cfg := syft.DefaultCreateSBOMConfig().
	...
	if err != nil {
		log.Fatalf("[FATAL] Failed to create Manifest: %v", err)
	}

	// r, err := ToSpdxSchema(s)
	r, err := ToCycloneDxSchema(s)
	if err != nil {
		log.Fatalf("[FATAL] Failed to convert to CycloneDX: %v", err)
	}

	filePath := "go-spdx.json"
	file, err := os.Create(filePath) // os.Create truncates if file exists
	if err != nil {
		log.Fatalf("[FATAL] Error creating file: %v", err)
	}
	defer func() {
		if err := file.Close(); err != nil {
			log.Printf("failed to close file: %v", err)
	...
	if err != nil {
		log.Fatalf("[FATAL] Error copying content: %v", err)
	}
	...
	log.Printf("[INFO] Manifest saved to: %s", filePath)
	log.Printf("[INFO] Vulnerabilities saved to: %s", vulnFilePath)
	return *s
}
