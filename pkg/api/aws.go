...
// trace/<org_id>/<image_id>/<filename>
...
func (b *S3KeyBuilder) BuildKey(filename string) string {
	return fmt.Sprintf("trace/%s/%s/%s", b.OrgID, b.ImageID, filename)
}

// BuildManifestKey creates an S3 key for Manifest file
func (b *S3KeyBuilder) BuildManifestKey() string {
	return b.BuildKey("manifest.json")
}
...
func (h *ScanHandler) ListImageScans(ctx context.Context, orgID, imageID string) ([]string, error) {
	...
	prefix := fmt.Sprintf("trace/%s/%s/", orgID, imageID)
	var files []string
